...
-- Case 1: Query by customer ID
SELECT * FROM order_status_transaction(1, 1, 123);

-- Case 2: Query by customer last name  
SELECT * FROM order_status_transaction(1, 1, NULL, 'SMITH');

-- Function to generate input data according to TPC-C specification
CREATE OR REPLACE FUNCTION generate_order_status_input(
    p_w_id INTEGER
) RETURNS TABLE(
    d_id INTEGER,
    c_id INTEGER,
...
DECLARE
    ...
    v_c_id INTEGER;
    v_c_last VARCHAR;
    v_nurand_result INTEGER;
BEGIN
    -- Generate random district ID [1..10]
    v_d_id := FLOOR(RANDOM() * 10) + 1;
    
    -- Generate random number y within [1..100]
    v_random_y := FLOOR(RANDOM() * 100) + 1;
    
    -- 60% by last name, 40% by customer number
    IF v_random_y <= 60 THEN
        -- Generate customer last name using NURand(255,0,999) equivalent
        v_nurand_result := (FLOOR(RANDOM() * 255) | FLOOR(RANDOM() * 1000)) % 1000;
        v_c_last := 'CUSTOMER_' || LPAD(v_nurand_result::TEXT, 3, '0');
        RETURN QUERY SELECT v_d_id, NULL::INTEGER, v_c_last;
    ELSE
        -- Generate customer number using NURand(1023,1,3000) equivalent
        v_c_id := (FLOOR(RANDOM() * 1023) | FLOOR(RANDOM() * 3000)) % 3000 + 1;
        RETURN QUERY SELECT v_d_id, v_c_id, NULL::VARCHAR;
    END IF;
END;
$$ LANGUAGE plpgsql;

-- Complete transaction with input generation
CREATE OR REPLACE FUNCTION execute_order_status_transaction(p_w_id INTEGER)
RETURNS VOID AS $$
DECLARE
    input_data RECORD;
BEGIN
    ...
    FROM generate_order_status_input(p_w_id);
    
    -- Execute the appropriate query based on input type
    IF input_data.c_id IS NOT NULL THEN
        PERFORM * FROM order_status_transaction(
            p_w_id, 
            input_data.d_id, 
            input_data.c_id, 
            NULL
    ...
    -- Transaction is automatically committed when function ends
END;
$$ LANGUAGE plpgsql;
...
-- 1.2 - CHATGPT  --
--------------------
-- Assumptions about table/column names (adjust if your schema differs):
-- CUSTOMER(C_W_ID, C_D_ID, C_ID, C_FIRST, C_MIDDLE, C_LAST, C_BALANCE, ...)
-- "ORDER"(O_W_ID, O_D_ID, O_ID, O_C_ID, O_ENTRY_D, O_CARRIER_ID, O_OL_CNT, ...)
...
BEGIN
    ...
    ELSE
        ...
        IF v_cnt = 0 THEN
            RAISE EXCEPTION 'Customer last name not found: W=% D=% LAST=%', p_w_id, p_d_id, p_c_last;
        END IF;

        -- offset = ceil(n/2) - 1  => (n - 1) / 2 truncated
        v_offset := (v_cnt - 1) / 2;

        -- select the customer at position ceil(n/2) in ASC by C_FIRST
        SELECT c_id, c_first, c_middle, c_last, c_balance
        INTO v_customer
        FROM customer
        WHERE c_w_id = p_w_id
    ...
    -- 2) Find the customer's last order (largest O_ID)
    SELECT o_id, o_entry_d, o_carrier_id
    INTO v_order
    FROM "order"
    WHERE o_w_id = p_w_id
    ...
    IF NOT FOUND THEN
        -- Customer has no orders; return customer info and empty order info/lines
    ...
    -- 3) Collect order lines for that order into JSON array
    SELECT jsonb_agg(
             jsonb_build_object(
               ...
               'OL_AMOUNT',      ol_amount,
               'OL_DELIVERY_D',  ol_delivery_d  -- will be null if not delivered
             )
           ORDER BY ol_number
           )
    INTO v_lines
    ...
    -- 4) Return a single JSON object containing non-repeating and repeating groups
    RETURN jsonb_build_object(
        'W_ID', p_w_id,
        'D_ID', p_d_id,
        'C_ID', v_customer.c_id,
...
-- 1.3 - GEMINI  --
-------------------
-- Assumes the following table definitions exist, consistent with TPC-C:
-- WAREHOUSE, DISTRICT, CUSTOMER, HISTORY, NEW_ORDER, ORDER, ORDER_LINE, ITEM, STOCK

CREATE OR REPLACE PROCEDURE ORDER_STATUS_TX(
    IN p_w_id INT,
    IN p_d_id INT,
...
DECLARE
    -- Internal variable for selected C_ID
    v_c_id INT;
BEGIN
    -- Start Transaction Block (implicitly handled by DO block or connection settings, 
    -- but explicit COMMIT/ROLLBACK is not used here as per TPC-C ACID note)
    ...
    ELSIF p_c_last IS NOT NULL THEN
        -- Case 2: Customer selected by C_LAST (Middle customer logic)
        ...
        FROM
            SelectedCustomers
        WHERE
            rn = CEIL(total_count / 2.0); -- Position n/2 rounded up
    ...
    IF out_o_id IS NULL THEN
        -- Customer has no orders (unlikely in TPC-C, but good practice)
        -- TPC-C driver logic typically ensures last order exists.
    ...
    ORDER BY
        ol_i_id ASC; -- Order by item ID for a consistent display
